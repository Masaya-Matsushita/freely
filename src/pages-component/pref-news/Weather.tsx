...
export const Weather: FC<{ data: WeatherObj }> = (props) => {
  ...
  console.log(props.data.weatherTokyo)
    ...
    <div className='space-y-4'>
      ...
      {osaka ? (
        <Card>
          ...
          <div>タイムゾーン:{osaka.list[0].dt_txt}</div>
          <div>天気アイコン:{osaka.list[0].weather[0].icon}</div>
          <div>天気ひとこと:{osaka.list[0].weather[0].description}</div>
          <div>風速:{osaka.list[0].wind.speed}</div>
          <div>風向:{osaka.list[0].wind.deg}</div>
          <div>気温:{osaka.list[0].main.feels_like}</div>
          <div>最高気温:{osaka.list[0].main.temp_max}</div>
          <div>最低気温:{osaka.list[0].main.temp_min}</div>
          <div>湿度:{osaka.list[0].main.humidity}</div>
        </Card>
      ) : null}
    </div>
  )
